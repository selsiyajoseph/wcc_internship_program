...
function downloadRecording() {
    ...
    // Check for extend session ID (from Huddle extend)
    chrome.storage.local.get(['extendSessionId'], (result) => {
        const sessionId = result.extendSessionId;
        
        let filename;
        if (sessionId) {
            // Save inside the same folder as Huddle
            filename = `${sessionId}/${currentService}-recording-${timestamp}.webm`;
            console.log(`📁 Meet recording saving to folder: ${filename}`);
            
            // Clear the session ID after use
            chrome.storage.local.remove(['extendSessionId']);
        } else {
            // Save directly to Downloads (no folder)
            filename = `${currentService}-recording-${timestamp}.webm`;
            console.log(`📁 Meet recording saving directly to Downloads`);
        }
        ...
        chrome.downloads.download({ url, filename, saveAs: false }, (downloadId) => {
            if (chrome.runtime.lastError) {
                ...
                console.log("✅ DOWNLOAD started with ID:", downloadId, "→", filename);
            }
