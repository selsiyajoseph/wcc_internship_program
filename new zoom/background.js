...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  console.log("🔇🔇🔇 BACKGROUND: MUTE STATE RECEIVED →", message.muted ? "MUTED" : "UNMUTED");
  console.log("🔇 Sender tab:", sender.tab?.id, sender.tab?.url);
  
  // Relay to ALL recorder tabs
  chrome.tabs.query({}, (tabs) => {
    console.log(`🔇 Searching for recorder tabs among ${tabs.length} tabs...`);
    
    const recorderTabs = tabs.filter(tab => 
      tab.url && tab.url.includes("recorder.html")
    );
    
    console.log(`🔇 Found ${recorderTabs.length} recorder tabs`);
    
    recorderTabs.forEach(tab => {
      console.log(`🔇 Forwarding mute state to recorder tab: ${tab.id}`);
      chrome.tabs.sendMessage(tab.id, {
        action: "zoomMuteChanged",
        muted: message.muted
      }, (response) => {
        if (chrome.runtime.lastError) {
          console.error("❌ Failed to forward mute to recorder:", chrome.runtime.lastError);
        } else {
          console.log("✅ Mute forwarded to recorder successfully");
        }
      });
    });
  });
